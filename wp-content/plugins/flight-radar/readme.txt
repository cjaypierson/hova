=== Flight Radar ===

Contributors: beliefmedia
Donate link: http://www.flight.org/
Tags: aviation, aircraft, aerospace, shortcode, radar, airports, airplane, atc, air traffic control, flight
Requires at least: 3.1
Tested up to: 4.0
Stable tag: 0.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Generate FlightRadar24 Radar plots with the use of shortcode.

== Description ==

**Flight Radar** is plugin that simply generates FlightRadar24.com radar plots on your posts or pages with shortcode. 

Features include:

* Numerous shortcode options (list, random, date, formatting ect).

The initial purpose was to provide an easy means for people to provide radar plots on [Flight.org](http://www.flight.org/ "Flight"). Featrues are very basic but will be built upon in future releases.

Related links:

* [Usage](http://www.beliefmedia.com/wp-plugins/flightradar.php)
* [Flight](http://www.flight.org/flight-radar-24-wordpress-plugin)

== Installation ==

**To install the plugin manually:**

1. Extract the contents of the archive (zip file)
2. Upload the flight-radar folder to your '/wp-content/plugins' folder
3. Activate the plugin through the Plugins section in your WordPress admin panel.

**Upload via the WordPress administration panel:**

1. Click on "Plugins" in the left panel, then click on "Add new".
2. You should now see the Install Plugins page. Click on "Upload".
3. Click on Browse and select your "flight-radar.zip" file.
4. Click on "Install now", activate it and you're done!

== Screenshots ==

1. Example Radar Plot Rendered in a WordPress Post.

== Changelog ==

= 0.1 =

* Initial Release. Basic features only.

== Upgrade Notice == 

= 0.1 =
Initial Release. Basic features only.

== Frequently Asked Questions ==

**What will the plugin do?**

The plugin is in very early stages. It will retrieve radar plots from FlightRadar24 and embed an iframe on your website. You can call the plot by either latitude and longitude, or by just referencing an ICAO/IATA airport code.

**Where can I go for support?**

View the complete FAQs and other information [here](http://www.beliefmedia.com/wp-plugins/flightradar.php). 